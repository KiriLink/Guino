...
class UserSettingsController extends Controller
{
    ...
    {
        // Lógica para actualizar las vidas del usuario
        // Aquí deberías tener la lógica que incrementa las vidas y actualiza la marca_tiempo

        $nuevasVidas = auth()->user()->vidas;

        $marcaTiempo = auth()->user()->marca_tiempo;

        $subscripcion = auth()->user()->subscripcion;
        
        return response()->json(['vidas' => $nuevasVidas, 'marca_tiempo' => $marcaTiempo, 'subscripcion'=> $subscripcion]);
    }

    public function comprarVida()
    {
        $updateData['vidas'] = auth()->user()->vidas + 1;
    ...
    public function restarVida()
    {
        ...
        // Verificar si el usuario existe y tiene vidas disponibles
        if ($usuario && $usuario->vidas > 0) {
            $updateData['vidas'] = auth()->user()->vidas - 1;
        ...
        DB::table('users')
            ...
            return response()->json(['mensaje' => 'Vida reducida correctamente']);
        } else {
            // Manejar el caso en el que el usuario no tenga vidas disponibles
            return response()->json(['mensaje' => 'No hay vidas disponibles para reducir']);
        }
    }
    public function comprarSubscripcion($dato)
    {
        if ($dato == 1) {
            $updateData['subscripcion'] = 1;
        } else {
            $updateData['subscripcion'] = 0;
        }
        DB::table('users')
            ->where('id', auth()->user()->id)
            ->update($updateData);
